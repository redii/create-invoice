...
func main() {
  ...
  pdf.MultiCell(60, 5, tr(data[0]), borderMode, "L", false)
	pdf.SetXY(xBeforeMeta, yBeforeMeta)

  // set invoice metadata (DATA)
	pdf.SetFont("Arial", "B", 10)
	pdf.Line(xBeforeMeta, yBeforeMeta, xBeforeMeta + 170, yBeforeMeta)
	pdf.CellFormat(110, 10, tr(data[1]), borderMode, 0, "L", false, 0, "")
  pdf.CellFormat(60, 10, tr(data[2]), borderMode, 0, "R", false, 0, "")
  pdf.Ln(12)
  ...
  pdf.Ln(6)
	...
	var total float64 = 0
	headerBool := true
	lines := strings.Split(string(data[3]), "\n")
  ...
  // output
	path := strings.Replace(tr(data[1]), " ", "_", -1) + ".pdf"
	log.Printf("Ouput: %v\n", path)
